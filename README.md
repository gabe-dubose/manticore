...
Manticore is an R library for implementing a Monte Carlo simulation approach for statistically assessing patterns of phylosymbiosis, which are
characterized by significant topological congruence between a host phylgeny and the dendrogram representing dissimilarity in their microbial communtiy 
compositions. The general idea is pretty simple. To statistically evaluate the congruence between two trees,
you can generate a null congruence distirbution by repeatedly simulating random trees and quantifying how congruent they are 
with your tree of interest. You can then ask the question: <i>how often is the congruence between my tree of interest and random trees
...
#### Dependencies 
Manticore depends on the following R libraries for internal functionality:
```
TreeDist
ape
phangorn
...
The previous example considers trees as fixed objects, and in doing so, assumes they were estimated correctly. However, there is usually 
uncertainty in tree estimation which could potentially influence inferences regarding a phylosymbiotic signal. To contextualize how 
susceptible said inference is to this uncertainty, we can instead generate a distirbution of possible congruence values by
using subtree pruning and regrafting to intorduce increasing degrees of noise into the observed trees. We can then evaluate to what extent
inferences are robust ($P(Null \geq Obs)$ values remain above $\alpha$) as a function of increasing noise ($\frac{n_{\text{SPRs}}}{n_{\text{leaves}}}$).
This functionality is offered by the \emph{rtc.sensitivity.test} function. Following our previous example, we can generate said sensitivity curve as:

```
sensitivity.curve <- rtc.sensitivity.test(tree1, tree2, rtc.results$null.congruence.model,
                                          'MCI', spr.proportions = seq(0, 0.5, by=0.05), iterations=100)

```

We can then plot this curve to see what degree of noise causes the integrated $P(Null \geq Obs)$ value to exceed $\alpha$:

```
plot(x=sensitivity.curve$prop.div, y=sensitivity.curve$integrated.p, type = "l",
     ylab = 'Integrated P(Null >= Obs.)', xlab = 'n SPRs / n leaves', lwd = 2)
abline(h = 0.05, col = "red", lty = 2, lwd = 2)
```
which shows:

<img src="images/demo_sensitivity.png" width="300"/>

As we can see, this line crosses $\alpha = 0.05$ between a noise level of $0.5 < \frac{n_{\text{SPRs}}}{n_{\text{leaves}}} < 0.1$, indicating that
if approximately 5 - 10% of the tree is estimated incorrectly, the results would no longer be robust. Interpreting this quantity would be best informed
by pairing it with emperical estimates of uncertainty in tree estimation, for example, generated by bootstrapping. 
...
Run a sensitivity test on random tree congruence test results. This function tests how sensitive RTC test results are to variation in congruence estimates (uncertainty). 
To accomplish this, a distribution of congruence values are generated by iteratively using subtree pruning (SPR) and regrafting to mutate the original input trees. The congruence distribution is then compared against the null model, and an integrated $P(Null \geq Observed)$ values is calculated across the congruence distribution. The extent of SPR is increased to introduce greater variation. 
...
null.congruence.model  Null model generated by rtc.test function

metric  Congruence metric specified for null model generation

spr.proportions  Vector of proportions of tree size to subtree prune and regraft: default = seq(0, 0.5, by=0.05)

iterations  Number of times to perform SPR on input trees to generate congruence distribution: default = 100
```

<b>Value</b>:

Function returns a dataframe containing the integrated P(Null >= Observed) (integrated.p column) as a function of SPR proportion (prop.dive column).
